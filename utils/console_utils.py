    ...
    def get_key():
        ...
        if os.name == 'nt':  # Windows
            ...
            try:
                ...
                try:
                    # Use setraw instead of cbreak for better compatibility
                    tty.setraw(fd)
                    key = sys.stdin.read(1)
                    if key == '\x1b':  # ESC sequence
                        # Set stdin to non-blocking to read escape sequences
                        import select
                ...
                # Fallback for systems without termios/tty or when not in terminal
                try:
                    key = input().strip()
                    if not key:
                        return 'ENTER'
    ...
    def _test_arrow_support(self) -> bool:
        ...
        try:
            if os.name == 'nt':
                ...
                import termios
                import tty
                # Test if we're in a proper terminal
